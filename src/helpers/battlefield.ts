...
export function mapOverBattlefield(
	...
	const newBattlefield = clone(EMPTY_BATTLEFIELD_ROWS)

	for (const lineType of enumKeys(BATTLEFIELD_LINE)) {
		const line = battlefield[lineType]
...
export function mapBattlefield<T>(
	battlefield: Battlefield,
