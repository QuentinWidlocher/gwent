...
export function getBattlefieldAfterModifiers(
	battlefield: Battlefield
): Battlefield {
	// We get a list of all the modifiers info from the battlefield
	const modifiers: [Modifier, PlacedCard, BATTLEFIELD_LINE][] = mapBattlefield(
		...
		(
			...
			line
				...
				.map(
					(card) =>
						[card.modifier, card, lineType] as [
							Modifier,
							PlacedCard,
							BATTLEFIELD_LINE
						]
	...
	const sortedModifiers = modifiers.sort(([m]) => m.priority).reverse()

	// We give all cards their original strength and we remove their modifiers
	const resetBattlefied = mapOverBattlefield(battlefield, (line) =>
		line.map((card) => ({
			...card,
	...
	const newBattlefield = sortedModifiers.reduce(
...
export async function getStateAfterPlayingCard(
	...
	let gameState: GameState = fromPlayerPov
		? currentGameState
		: swapPov(currentGameState)
	const linePlacedOnFromPov =
		fromPlayerPov || isNil(linePlacedOn)
			? linePlacedOn
			: swapLinePov(linePlacedOn)
	...
	if (
		notNil(card.canBePlayed) &&
		!card.canBePlayed(card, gameState, linePlacedOnFromPov)
	) {
	...
	if (notNil(linePlacedOnFromPov) && canBePlaced(card)) {
		const rowsWithCard: Battlefield = { ...gameState.battlefield }
	...
	if (notNil(card.onCardPlayed)) {
		...
		const infos = await card.onCardPlayed(
			card,
			gameState,
			linePlacedOnFromPov,
			cardPlacedOn,
	...
	return [
		true,
		fromPlayerPov ? gameState : swapPov(gameState),
		cardsToPlayNext,
	]
}
...
export function autoPlay(
	...
	const gameState: GameState = fromPlayerPov
		? currentGameState
		: swapPov(currentGameState)
	...
	const selectedCard =
		gameState.playerHand[
			Math.floor(Math.random() * gameState.playerHand.length)
		]
...
export function shouldEnemyPassTheTurn(
	...
	const playerHandStrength = sum(
		gameState.playerHand.filter(canBePlaced).map(getStrength)
	)
	const enemyHandStrength = sum(
		gameState.enemyHand.filter(canBePlaced).map(getStrength)
	)
	const totalPlacedCards = sum(
		mapBattlefield(gameState.battlefield, (l) => l.length)
	)
